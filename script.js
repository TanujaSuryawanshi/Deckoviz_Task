...
if (dd) {
  const link = dd.querySelector(".dropdown");
  const menu = dd.querySelector(".mega-menu");
  const cards = dd.querySelectorAll(".mega-card");
  const head = dd.querySelector(".mega-head");
  let open = false;
  const openMenu = () => {
    menu.classList.add("visible");
    gsap.fromTo(
      menu,
      { opacity: 0, y: 12, rotateX: -8, scale: 0.98 },
      {
        opacity: 1,
        y: 0,
        rotateX: 0,
  ...
  dd.addEventListener("mousemove", function (e) {
    ...
    const hx = (e.clientX - rect.left - rect.width / 2) / 20;
    const hy = (e.clientY - rect.top - rect.height / 2) / 20;
    menu.style.setProperty("--hx", hx + "px");
    menu.style.setProperty("--hy", hy + "px");
  });
...
const md = document.querySelector(".menu-dropdown-container");
if (md) {
  const btn = md.querySelector(".menu-btn");
  const panel = md.querySelector(".menu-dropdown");
  const items = md.querySelectorAll(".menu-item");
  let openM = false;
  const openPanel = () => {
    ...
    gsap.fromTo(
      items,
      { opacity: 0, y: 10 },
      { opacity: 1, y: 0, duration: 0.16, ease: "power2.out", stagger: 0.04 }
    );
    openM = true;
  };
  const closePanel = () => {
  ...
  md.addEventListener("mouseenter", openPanel);
  md.addEventListener("mouseleave", closePanel);
  md.addEventListener("mousemove", function (e) {
    const rect = panel.getBoundingClientRect();
    const hx = (e.clientX - rect.left - rect.width / 2) / 20;
    const hy = (e.clientY - rect.top - rect.height / 2) / 20;
    panel.style.setProperty("--hx", hx + "px");
    panel.style.setProperty("--hy", hy + "px");
  });
